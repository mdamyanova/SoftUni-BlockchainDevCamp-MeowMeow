...
public class CatContractServiceImpl implements CatContractService {
    ...
    @Override
    public AdoptionCatViewModel byName(String name) {
        // TODO - This is nasty
        AdoptionCat cat = this.repository.byName(name);
        return this.mapper.map(cat, AdoptionCatViewModel.class);
    ...
    @Override
    public void add(AdoptionCatBindingModel cat) throws Exception {
        TransactionReceipt txInfo = this.contract
                .add(
                        cat.getName(),
                        cat.getAge(),
